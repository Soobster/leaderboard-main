/*
The View Tier List Page. This shows a non-editable version of a created tierlist object
that anyone can view.

This page is loaded and shown when a user publishes/updates a tier list or clicks on a tier list preview component.
*/
import {
	Avatar,
	Card,
...
import { arrayRemove, deleteDoc, doc, getDoc, updateDoc, collection, setDoc, serverTimestamp, arrayUnion, query, getDocs, where } from "firebase/firestore"
...
function ViewTierListPage() {
	...
	// useEffect for getting list object
	useEffect(() => {
		getList()
	}, [listID])
	...
	async function getList() {
		...
		if (!userSnapshot.docs[0].data().tierlists.includes(listID)) navigate("/notfound")

		const listDocRef = doc(db, "tierlists", listID)
	...
	const handleDeleteList = async () => {
		...
		await updateDoc(userRef, { tierlists: arrayRemove(listID) })
	...
	const createComment = async () => {
		if (comment) {
			...
			await commentNotification(
				userID,
				list.data().userId,
				{ id: listID, name: list.data().name, collection: "tierlists", ownerID: list.data().userId },
				null
			)
	...
	function ShowList() {
		...
		Object.keys(list.data().list).map((key, index) =>
			rows.push(
				<div key={index}>
					<Stack direction="row">
						<Box
							bgcolor={list.data().list[key].color}
							mt={2}
							sx={{
								display: "flex",
								alignItems: "center",
								...
								width: { xs: 90, md: 150 },
								height: { xs: 105, md: 163 }
							}}>
							<ThemeProvider theme={theme}>
								<Typography fontSize={"32px"} sx={{ flexGrow: 1 }}>
									{list.data().list[key].name}
								</Typography>
							</ThemeProvider>
						</Box>

						<Grid container>
							{list.data().list[key].items.map((game, index) => {
								return (
				...
				for (let i = 0; i < comments.length; i++) {
					rows.push(
						<CommentComponent
							...
							listData={{ id: listID, name: list.data().name, collection: "tierlists", ownerID: list.data().userId }}
						/>
					)
				}
								...
								<>
									...
									<Grid item md={1} xs={2} ml={-1}>
										<Avatar sx={{ bgcolor: "#A1A1A1" }}>
											<Button onClick={() => navigate(`/${user.data().username}/tierlist/edit/${listID}`)} disabled={clickedDelete}>
								...
								<Grid container>
									...
									<Grid item md={1.5} xs={2} mt={4} align="center">
										<Avatar sx={{ bgcolor: upvote ? "#4C2F97" : "#A1A1A1" }}>
											<Button
												onClick={() =>
													manageListVote(1, "tierlists", userID, user.id, listID, setUpvote, setUpvotes, setDownvote, setDownvotes)
												}>
									...
									<Grid item xs={2} md={1.5} mt={4} align="center">
										<Avatar
											...
											<Button
												onClick={() =>
													manageListVote(0, "tierlists", userID, user.id, listID, setUpvote, setUpvotes, setDownvote, setDownvotes)
												}>
...
export default ViewTierListPage
